...
-- tainted can come back "secret" - comparing it raises a Lua error instead of
-- just returning something useless. canaccessvalue/issecretvalue only exist
-- where that concept does, so this is a harmless no-op on Classic Era/TBC/MoP.
...
local function compare(val1, comparator, val2)
	if not isAccessibleValue(val1) or not isAccessibleValue(val2) then
		return false
	end
